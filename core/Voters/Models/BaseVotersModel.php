...
class BaseVotersModel extends CoreModel
{
    ...
    {
        if ($this->table == 'voters_original') {
            return $this->queryOriginalVoter();
        }

        return $this->queryProcessedVoter();
    }

    private function queryOriginalVoter()
    {
        return $this->select([
            "{$this->table}.id",
            "{$this->table}.id as voters_original_id",
            "{$this->table}.nik",
            "{$this->table}.nkk",
            "{$this->table}.name",
